...
(() => {
  ...
  try {
    ...
    checkoutService(customers[0], cart); // Walter: 2000 - 100 = 1900
  } catch (e: any) {
    console.log("❌", e.message);
  }
})();

// ✅ Test 2: Buying shipping items (cheese + biscuits)
(() => {
  ...
  try {
    ...
    checkoutService(customers[1], cart); // Jessie: 1000 - 380 = 620
  } catch (e: any) {
    console.log("❌", e.message);
  }
})();
...
(() => {
  console.log("\n🔹 Test 3: Buying 100x tv (only 3 in stock)");
  ...
  try {
    ...
    console.log("❌", e.message); // Expect: "tv is out of stock"
  }
})();

// ❌ Test 4: Expired item (simulate expired milk)
(() => {
...
(() => {
  ...
  const cart = new Cart(cloneProducts());
  try {
    cart.add("laptop", 1); // Price: 7000
    checkoutService(customers[2], cart); // Saul has 500 only
  } catch (e: any) {
    console.log("❌", e.message); // Expect: "Customer does not have enough balance"
  }
})();
